...
import com.hbbproject.echo.alice.utils.IOUtils;
import com.hbbproject.echo.alice.utils.JapaneseUtils;
...
public class Chat {
   ...
   // public History<String> repetitionHistory = new History<String>("repetition");
    ...
    void addPredicates() {
        try {
            predicates.getPredicateDefaults(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath()+"/custom/config/predicates.txt") ;
        } catch (Exception ex)  {
            ex.printStackTrace();
        }
    }
