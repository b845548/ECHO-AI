...
import com.hbbproject.echo.alice.Alice;
...
//    private static ChatArrayAdapter adapter;
    ...
    String bot_name_path="custom"; 
    String aiml_path = bot_name_path+"/aiml";
    String sets_path = bot_name_path+"/sets";
    String maps_path = bot_name_path+"/maps";
    Context c=getApplicationContext();
            File file = new File(    Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath()+"/custom/config/predicates.txt");
       if (file.exists()) {
     //    Toast.makeText(MainActivity.this, "exist", Toast.LENGTH_SHORT).show();
        }else{
       // Toast.makeText(MainActivity.this, "exist pas", Toast.LENGTH_SHORT).show();
    }
   try{
        File f = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath()+"/custom/sets");
        String[] listOfFiles=f.list();
        loading+=listOfFiles.length;
        f = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath()+"/custom/maps");
        listOfFiles=f.list();
        loading+=listOfFiles.length;
        f = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath()+"/custom/aiml");
        listOfFiles=f.list();
        loading+=listOfFiles.length;
    }catch(Exception e){
            e.printStackTrace();
...
//                        Toast.makeText(MainActivity.this, "brain loading", Toast.LENGTH_SHORT).show();
  ...
  //                      Toast.makeText(MainActivity.this, "brain loaded", Toast.LENGTH_SHORT).show();
                         tv1.setText("Salut, je suis ChatBot!");
            ...
            if (intent.getAction().equalsIgnoreCase(Constants.BROADCAST_ACTION_BRAIN_ANSWER)) {
                ...
                if(answer.contains("I have no answer"))answer="Je n'ai pas de reponse";                
                edt.setHint("Dis moi"); 
    ...
    private void setCurrentInflateItem(int type){
        if(type==0){
            ...
            pageActuel=1;
            if(tutorial==false)
            textToSpeech.speak("Tu veux discuter avec moi?", TextToSpeech.QUEUE_FLUSH, null);
      
        }else if(type==2){
            mPager.setCurrentItem(2);
            pageActuel=2;
            if(tutorial==false)
                textToSpeech.speak("Dis moi", TextToSpeech.QUEUE_FLUSH, null);
        }
    }
    ...
    private class PagerAdapterClass extends PagerAdapter{
        ...
        public Object instantiateItem(View pager, int position) {
            ...
            if(position==0){
                ...
                textToSpeech.speak("Donnes moi ton message", TextToSpeech.QUEUE_FLUSH, null);
            ...
            messageReceive.setText("Je ne peux pas appeller cette personne");   
            textToSpeech.speak("Je ne peux pas appeller cette personne", TextToSpeech.QUEUE_FLUSH, null);
            startActivity(intent);
            }else{
            messageReceive.setText("Je ne connais pas cette personne"); 
            textToSpeech.speak("Je ne connais pas cette personne", TextToSpeech.QUEUE_FLUSH, null);
            }
            ...
            if(playingMusic==false){
                messageReceive.setText("Je vais mettre une musique"); 
                textToSpeech.speak("Je vais mettre une musique", TextToSpeech.QUEUE_FLUSH, null);
                startService(new Intent("com.service.test"));
                playingMusic=true;
            }else{
                messageReceive.setText("Si tu veux changer cette musique? dis 'arretes'");  
                textToSpeech.speak("Si tu veux changer cette musique? dis 'arretes'", TextToSpeech.QUEUE_FLUSH, null);
            }
                ...
                if(playingMusic==true){
                    messageReceive.setText("Je m'arrête :)");    
                    textToSpeech.speak("Je m'arrête", TextToSpeech.QUEUE_FLUSH, null);
                    stopService(new Intent("com.service.test"));
                    playingMusic=false;
                }else{
                    messageReceive.setText("Je n'ai pas mis la musique");  
                    textToSpeech.speak("Je n'ai pas mis la musique", TextToSpeech.QUEUE_FLUSH, null);
                }
