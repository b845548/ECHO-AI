...
//        }
  ...
  //File f = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MUSIC).getAbsolutePath());//Alice.getContext().getAssets().list(aiml_path);
            String[] listOfFiles = Alice.getContext().getAssets().list(aiml_path);
            if (MagicBooleans.trace_mode){
                System.out.println("Loading AIML files from "+aiml_path+listOfFiles[0]+listOfFiles[1]);
                BrainLogger.getInstance().info("Loading AIML files from "+aiml_path+listOfFiles[2]+listOfFiles[3]);
            }
   ...
   /* public void writeUnfinishedIFCategories() {
    ...
    int addAIMLSets() {
        ...
        try {
            ...
            if (MagicBooleans.trace_mode){
                System.out.println("Loading AIML Sets files from " + sets_path+listOfFiles[0]);
                BrainLogger.getInstance().info("Loading AIML Sets files from " + sets_path+listOfFiles[1]);
            }
